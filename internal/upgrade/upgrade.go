...
const releaseURL = "https://api.github.com/repos/VladGavrila/gocli-gen/releases/latest"
...
func Run(currentVersion string) error {
	...
	tmpFile, err := os.CreateTemp(dir, ".gocli-gen-upgrade-*")
	if err != nil {
	...
	if err := os.Rename(tmpPath, binaryPath); err != nil {
		return fmt.Errorf("failed to replace binary (try: sudo gocli-gen --upgrade): %w", err)
	}

	// 7. Success.
	fmt.Printf("Updated gocli-gen to v%s\n", latestVersion)
	return nil
}
...
func assetName() (string, error) {
	...
	case "darwin/arm64":
		return "gocli-gen-macos-arm64", nil
	case "linux/amd64":
		return "gocli-gen-linux-amd64", nil
	default:
